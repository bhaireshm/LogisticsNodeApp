        ...
        .then((savedEcom_attributevaluepairlisttype) => {
          if (!savedEcom_attributevaluepairlisttype.status) {
            res
              .status(400)
              .json(
                CustomResponse.APIErrorResponse(
                  savedEcom_attributevaluepairlisttype.message
                )
              );
          }
...
Ecom_attributevaluepairlisttypeController.deleteEcom_attributevaluepairlisttype =
  async (req, res) => {
    try {
      ...
      res.json(
        CustomResponse.deleteResponseFormat(
          removedEcom_attributevaluepairlisttype
        )
      );
    } catch (ex) {
      res.status(400).json(CustomResponse.APIErrorResponse(ex, ex.message));
        ...
        .then((updatedEcom_attributevaluepairlisttype) => {
          if (!updatedEcom_attributevaluepairlisttype.status) {
            res
              ...
              .json(
                CustomResponse.APIErrorResponse(
                  "Ecom Attribute Value PairList failed."
                )
              );
          } else {
            res.json(
